...
kumite.musicdraw.TestWorker = function(p) {
	if( p === $_ ) return;
	this.effect = new kumite.musicdraw.SquareEffect();
}
kumite.musicdraw.TestWorker.__name__ = ["kumite","musicdraw","TestWorker"];
kumite.musicdraw.TestWorker.main = function() {
	var instance = new kumite.musicdraw.TestWorker();
	onmessage = function(e){instance.start(webkitPostMessage, e);}
	proceed = function(e){instance.proceed(webkitPostMessage, e);}
}
kumite.musicdraw.TestWorker.prototype.effect = null;
kumite.musicdraw.TestWorker.prototype.start = function(postMessage,e) {
	onmessage = proceed;
	var result = new Uint8Array(10);
	var sendCommand = function(type,message) {
		postMessage(haxe.Serializer.run(new bpmjs.WorkerCommand(type)));
		postMessage(message);
	};
	var sendTransferableCommand = function(type,message) {
		postMessage(haxe.Serializer.run(new bpmjs.WorkerCommand(type)));
		postMessage(message.buffer,[message.buffer]);
	};
	this.effect.analyzer = haxe.Unserializer.run(e.data);
	this.effect.sendCommand = sendCommand;
	this.effect.sendTransferableCommand = sendTransferableCommand;
	this.effect.init();
	this.effect.render();
}
kumite.musicdraw.TestWorker.prototype.proceed = function(postMessage,e) {
	this.effect.gltexture.array = new Uint8Array(e.data);
	this.effect.render();
}
kumite.musicdraw.TestWorker.prototype.__class__ = kumite.musicdraw.TestWorker;
...
kumite.musicdraw.SquareEffect = function(p) {
	if( p === $_ ) return;
	this.rasterX = 0;
	this.gltexture = new GLArrayTexture();
	this.gltexture.width = 512;
	this.gltexture.height = 1024;
	this.gltexture.array = new Uint8Array(this.gltexture.width * this.gltexture.height * 4);
}
...
kumite.musicdraw.SquareEffect.prototype.sendCommand = null;
kumite.musicdraw.SquareEffect.prototype.sendTransferableCommand = null;
kumite.musicdraw.SquareEffect.prototype.gltexture = null;
...
kumite.musicdraw.SquareEffect.prototype.render = function() {
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			var y = _g3++;
			var x2 = x - Math.sin(ms / 176000) * this.gltexture.width - this.gltexture.width / 2 + this.rasterX;
			var y2 = y - Math.sin(ms / 299000) * this.gltexture.height - this.gltexture.height / 2;
			this.gltexture.setPixel(x,y,(Math.sin((x2 * x2 * y2 * y2 * 0.00000001 - ms / 5000) * Math.PI * 2) * 0.5 + 0.5) * 255,y,x,255);
		}
	}
	this.sendTransferableCommand("setResult",this.gltexture.array);
}
...
js.Lib.onerror = null;
kumite.musicdraw.TestWorker.main()