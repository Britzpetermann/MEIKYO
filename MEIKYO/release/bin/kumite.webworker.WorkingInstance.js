...
kumite.webworker.WorkingInstance = function(p) {
}
kumite.webworker.WorkingInstance.__name__ = ["kumite","webworker","WorkingInstance"];
kumite.webworker.WorkingInstance.main = function() {
	var instance = new kumite.webworker.WorkingInstance();
	onmessage = function(e){instance.onMessage(postMessage, e);}
}
kumite.webworker.WorkingInstance.prototype.onMessage = function(postMessage,e) {
	var x = 1.5243;
	postMessage("data: " + e.data);
	var v = haxe.Unserializer.run(e.data);
	v.multiply(2,2);
	var x1 = 0.0;
	var _g = 0;
	while(_g < 5000000) {
	...
	postMessage("!soso!" + x1 + " " + Std.string(v));
	var _g = 0;
	while(_g < 5000000) {
		var i = _g++;
		x1 += Math.sin(i);
...
kumite.webworker.WorkingInstance.prototype.__class__ = kumite.webworker.WorkingInstance;
Vec2 = function(x,y) {
	if( x === $_ ) return;
	this.x = x;
	this.y = y;
...
Matrix4.i43 = 11;
Matrix4.i44 = 15;
kumite.webworker.WorkingInstance.main()